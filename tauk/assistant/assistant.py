...
class TaukCompanion:
    ...
    def register_browser(self, debugger_address):
        logger.debug(f'[Companion] Registering browser {debugger_address}')
    ...
    def unregister_browser(self, debugger_address):
        logger.debug(f'[Companion] Unregistering browser {debugger_address}')
        url = f'http://localhost:{self._companion_port}/cdp/browser/new/{debugger_address}'
        response = requests.delete(url)
        if response.status_code != 200:
            logger.error(f'[Companion] Failed to unregister browser. Response: {response.text}')
